...
def dense_knn_matrix(x, k=16, relative_pos=None):
    ...
    with torch.no_grad():
        ...
        # # adj_matrices, edge_matrices, edge_names, TS, Edge_features = [], [], [], [], []
        ...
        # Edge_features = []
        #
        # device_index = 0  # Assuming a single GPU setup, set to appropriate index for multiple GPUs
        #
        # with ProcessPoolExecutor() as executor:
        #     futures = [executor.submit(process_batch, i, nn_idx, center_idx, x, n_points, batch_size, device_index) for
        #                i in range(batch_size)]
        #
        #     for future in tqdm(futures):
        #         DAD, edge_name, eDAD, T, edge_features = future.result()
        #         adj_matrices.append(DAD)
        #         edge_names.append(edge_name)
        #         edge_matrices.append(eDAD)
        #         TS.append(T)
        #         Edge_features.append(edge_features)
        # 创建一个空列表来存储所有的 DAD 矩阵
        all_deg_inv_sqrt = []
        all_deg_inv_sqrt_edge = []
        # 遍历每个样本
        # for i in tqdm(range(batch_size)):
        #     # 直接使用 view 而不是 numpy()
        #     current_nn_idx = nn_idx[i].reshape(-1)
        #     current_center_idx = center_idx[i].reshape(-1)
        #
        #     # 创建稀疏张量
        #     values = torch.ones(current_nn_idx.size(0), dtype=torch.float, device=x.device )  # 所有的连接权重都设为1
        #     adj_matrix = SparseTensor(row=current_center_idx, col=current_nn_idx, value=values,
        #                               sparse_sizes=(n_points, n_points))
        #
        #     # 对称化稀疏矩阵
        #     adj_matrix = adj_matrix.set_value_(None)  # 清除值以进行布尔运算
        #     adj_matrix = adj_matrix.add(adj_matrix.t())  # 进行转置和加法
        #     adj_matrix = adj_matrix.coalesce()  # 合并重复的元素
        #     adj_matrix = adj_matrix.set_value_(torch.ones(adj_matrix.nnz(), dtype=torch.float, device=x.device))  # 重新设置值
        #
        #     # 归一化
        #     deg = adj_matrix.sum(dim=1).to_dense()  # 计算度
        #     deg_inv_sqrt = deg.pow_(-0.5)
        #     deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
        #     # all_deg_inv_sqrt.append(deg_inv_sqrt)
        #     DAD = deg_inv_sqrt.view(-1, 1) * adj_matrix * deg_inv_sqrt.view(1, -1)
        #     adj_matrices.append(DAD)
        #
        #
        #
        #
        #     # 假设 `adj_matrix` 是一个 SparseTensor 对象
        #     # 从 SparseTensor 中提取非零元素的索引
        #     adj_matrix = adj_matrix.set_diag(0)
        #     edge_index = adj_matrix.coo()
        #
        #     # 计算边的数量
        ...
        #
        #     # 创建边的邻接矩阵
        #     edge_adj = torch.zeros((num_edge, num_edge), dtype=torch.bool, device=edge_index[0].device)
        #
        #     # 使用广播和向量化操作来找到共享节点的边
        #     src_nodes, dst_nodes = edge_index[0], edge_index[1]
        #     src_nodes_repeated = src_nodes.repeat_interleave(num_edge)
        #     dst_nodes_repeated = dst_nodes.repeat_interleave(num_edge)
        #     src_nodes_tiled = src_nodes.repeat(num_edge)
        #     dst_nodes_tiled = dst_nodes.repeat(num_edge)
        #
        #     # 构建共享节点的掩码
        #     shared_src = torch.eq(src_nodes_repeated, src_nodes_tiled).view(num_edge, num_edge)
        #     shared_dst = torch.eq(dst_nodes_repeated, dst_nodes_tiled).view(num_edge, num_edge)
        #
        #     # 如果两个边共享一个源节点或目标节点，则它们相邻
        #     edge_adj = shared_src | shared_dst
        #
        #     # 使矩阵对称
        #     edge_adj = edge_adj.logical_or(edge_adj.t())
        #
        #     # 填充对角线
        #     torch.diagonal(edge_adj).fill_(True)
        #
        #
        #     # 转换布尔矩阵为浮点数矩阵
        #     edge_adj = edge_adj.to(torch.float)
        #
        #
        #
        #     # 由于已经是SparseTensor格式，可以直接使用
        #     edge_adj_sparse = SparseTensor(row=edge_adj.nonzero()[:, 0], col=edge_adj.nonzero()[:, 1],
        #                                    value=edge_adj[edge_adj.nonzero(as_tuple=True)], sparse_sizes=edge_adj.shape)
        #
        #     # 归一化
        #     deg = edge_adj_sparse.sum(dim=1)
        #     deg_inv_sqrt = deg.pow_(-0.5)
        #     deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
        #     # all_deg_inv_sqrt_edge.append(deg_inv_sqrt)
        #     eDAD = deg_inv_sqrt.view(-1, 1) * edge_adj_sparse * deg_inv_sqrt.view(1, -1)
        #
        #     # 创建 edge_name 列表，存储每条边的源节点和目标节点
        #     edge_name = [(int(src), int(dst)) for src, dst in zip(edge_index[0], edge_index[1])]
        #
        #     edge_names.append(edge_name)
        #     # 添加到列表中
        ...
        #     # 获取索引和值
        #     values = adj_matrix.storage.value()
        #     indices = torch.stack([adj_matrix.storage.row(), adj_matrix.storage.col()], dim=0)
        #
        #     # 移除对角线元素
        #     mask = indices[0] != indices[1]
        #     values = values[mask]
        #     indices = indices[:, mask]
        #
        #     # 更新 adj_matrix，这里假设 adj_matrix 支持就地修改
        #     adj_matrix = SparseTensor(row=indices[0], col=indices[1], value=values, sparse_sizes=adj_matrix.sizes())
        #
        #     # 获取上三角矩阵的非零元素的索引
        #     upper_tri_indices = indices
        ...
        #
        #     # 创建数据，这里我们使用 ones
        #     data = torch.ones(num_edge * 2)
        #
        #     # 创建稀疏张量 T，注意这里直接使用已有的索引和值，避免重复创建
        #     # row_index 和 col_index 需要根据具体需求生成，这里假设它们已经被正确计算
        #     row_index = [item for sublist in nedge_name for item in sublist]
        #     col_index = list(range(num_edge)) * 2
        #
        #     # 创建稀疏张量 T，注意这里直接使用已有的 indices 和 values 创建 T，避免重复计算
        #     T = SparseTensor(row=torch.tensor(row_index),
        #                      col=torch.tensor(col_index),
        #                      value=data,
        #                      sparse_sizes=(adj_matrix.size(0), num_edge))
        #
        #     # 如果需要，可以将 T 转换回 torch.sparse_coo_tensor
        #     # T = T.to_torch_sparse_coo_tensor()
        #
        #     # 现在 T 是一个新的 SparseTensor，可以添加到列表中
        #     TS.append(T)
        #
        #     # 计算余弦距离
        #     x_normalized = torch.nn.functional.normalize(x[i], p=2, dim=1)
        ...
        #
        #     # 方向特征
        #     # 假设 directed_adj 是一个完整的邻接矩阵
        #     # 这里我们构建一个全零的矩阵，然后填充上三角非对角线部分
        #     directed_adj = torch.zeros_like(distance)
        #     directed_adj[upper_tri_indices[0], upper_tri_indices[1]] = 1
        #
        #     # 初始化方向特征张量
        #     direction_feat = torch.zeros((num_edge, 2), dtype=torch.int, device=x.device)
        #
        #     # 遍历每一条边
        #     for s in range(num_edge):
        #         src, dst = upper_tri_indices[0][s], upper_tri_indices[1][s]
        #
        #         # 检查边的方向
        #         if directed_adj[src, dst] == 1:
        #             direction_feat[s] = torch.tensor([1, 0])
        #         elif directed_adj[dst, src] == 1:  # 如果是反方向的边
        #             direction_feat[s] = torch.tensor([0, 1])
        #         else:
        #             raise ValueError("Edge not found in the directed adjacency matrix")
        #
        #     edge_features = torch.cat([edge_feat, direction_feat], dim=1)
        #     Edge_features.append(edge_features)
        # 在循环外进行归一化操作
        ...
        #     current_center_idx = center_idx[i].reshape(-1)
        #
        #     values = torch.ones(current_nn_idx.size(0), dtype=torch.float, device=x.device)
        #     adj_matrix = SparseTensor(row=current_center_idx, col=current_nn_idx, value=values,
        #                               sparse_sizes=(n_points, n_points))
        #
        #     adj_matrix = adj_matrix.set_value_(None).add(adj_matrix.t()).coalesce()
        #     adj_matrix = adj_matrix.set_value_(torch.ones(adj_matrix.nnz(), dtype=torch.float, device=x.device))
        #
        #     deg = adj_matrix.sum(dim=1).to_dense()
        #     deg_inv_sqrt = deg.pow_(-0.5)
        #     deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
        ...
        #     col_index = list(range(num_edge)) * 2
        #     data = torch.ones(num_edge * 2)
        #     T = SparseTensor(row=torch.tensor(row_index), col=torch.tensor(col_index), value=data,
        #                      sparse_sizes=(adj_matrix.size(0), num_edge))
        #     TS.append(T)
        #
        #     x_normalized = torch.nn.functional.normalize(x[i], p=2, dim=1)
        ...
        #         src, dst = upper_tri_indices[0][s], upper_tri_indices[1][s]
        #         if directed_adj[src, dst] == 1:
        #             direction_feat[s] = torch.tensor([1, 0], device=x.device)
        #         elif directed_adj[dst, src] == 1:
        #             direction_feat[s] = torch.tensor([0, 1], device=x.device)
        #         else:
        #             raise ValueError("Edge not found in the directed adjacency matrix")
        #
        #     edge_features = torch.cat([edge_feat, direction_feat], dim=1)
        #     Edge_features.append(edge_features)
        # for i, deg_inv_sqrt in enumerate(all_deg_inv_sqrt):
        #     DAD = deg_inv_sqrt.view(-1, 1) * adj_matrices[i] * deg_inv_sqrt.view(1, -1)
        #     adj_matrices[i] = DAD
        #
        # for i, deg_inv_sqrt in enumerate(all_deg_inv_sqrt_edge):
        #     eDAD = deg_inv_sqrt.view(-1, 1) * adj_matrices[i] * deg_inv_sqrt.view(1, -1)
        #     edge_matrices[i] = eDAD
