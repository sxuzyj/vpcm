...
class CustomModel(nn.Module):
    def __init__(self, base_model):
        ...
        # self.base_model = nn.Sequential(*list(base_model.children())[:-1])  # 去掉最后的全连接层
        self.base_model =base_model
        # self.fc = nn.Linear(base_model.num_features, 512)  # 自定义的全连接层
        # self.relu = nn.ReLU()
        # self.out = nn.Linear(512, 10)  # 最终输出10个类别

        dim = 258

        self.prediction_t = Seq(nn.Conv2d(dim, 1024, 1, bias=True),
                                nn.BatchNorm2d(1024),
                                act_layer('gelu'),
                                nn.Dropout(0.0),
                                nn.Conv2d(1024, 6, 1, bias=True))
        self.prediction_p = Seq(nn.Conv2d(dim, 1024, 1, bias=True),
                              nn.BatchNorm2d(1024),
                              act_layer('gelu'),
                              nn.Dropout(0.0),
                              nn.Conv2d(1024, 6, 1, bias=True))

        self.lstm = PatternLstm(1, dim)

        # self.raw_weights = nn.Parameter(torch.tensor([0.5, 0.5, 0.5]))
        self.alpha = nn.Parameter(torch.tensor([0.5]))
        # self.a = nn.Parameter(torch.Tensor(1,dim,1,1))
        # # 初始化参数
        # nn.init.normal_(self.a)
        #
        # self.b = nn.Parameter(torch.Tensor(1,dim,1,1))
        # # 初始化参数
        # nn.init.normal_(self.b)
        #
        # self.c = nn.Parameter(torch.Tensor(1,dim,1,1))
        # # 初始化参数
        # nn.init.normal_(self.c)

    def forward(self, image, t, p, need_p_t):
        ...
        return x, self.prediction_t(t).squeeze(-1).squeeze(-1), self.prediction_p(p).squeeze(-1).squeeze(-1)
